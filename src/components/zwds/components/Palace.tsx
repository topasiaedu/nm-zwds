...
const Palace: React.FC<PalaceProps> = ({
  ...
  const showAnnualFlow = palace.annualFlow && palace.annualFlow.year === targetYear;
  ...
  return (
    <motion.div 
          ...
          <motion.div 
            ...
            animate={{ 
              ...
              boxShadow: isSelected 
                ? ["0 0 0px rgba(255, 255, 255, 0.5)", "0 0 8px rgba(255, 255, 255, 0.3)", "0 0 0px rgba(255, 255, 255, 0.5)"] 
                : ["0 0 0px rgba(79, 70, 229, 0.3)", "0 0 6px rgba(79, 70, 229, 0.2)", "0 0 0px rgba(79, 70, 229, 0.3)"]
            }}
            transition={{ 
              duration: 0.5, 
              ease: "easeOut",
      ...
      <div className="absolute bottom-0 left-0 right-0 grid grid-cols-3 w-full text-3xs xs:text-2xs sm:text-xs text-zinc-800 dark:text-zinc-200 border-t border-gray-200 dark:border-gray-700 z-20">
        ...
        <div className={`flex flex-col items-center justify-center py-0.5 xs:py-1 sm:py-1.5 border-r border-gray-200 dark:border-gray-700 ${isSelected ? "text-white/90 dark:text-white/90" : ""}`}>
          <div>{language === "en" && t(`zwds.stems.${palace.heavenlyStem}`) ? t(`zwds.stems.${palace.heavenlyStem}`) : palace.heavenlyStem}</div>
          <div>{language === "en" && t(`zwds.branches.${palace.earthlyBranch}`) ? t(`zwds.branches.${palace.earthlyBranch}`) : palace.earthlyBranch}</div>
        </div>
        
        {/* Bottom middle: Palace Name and Age Range */}
        <div className={`flex flex-col items-center justify-center py-0.5 xs:py-1 sm:py-1.5 border-r border-gray-200 dark:border-gray-700 font-medium ${isSelected ? "text-white dark:text-white" : ""}`}>
          <div>{language === "en" && t(`zwds.palaces.${palace.name}`) ? t(`zwds.palaces.${palace.name}`) : palace.name}</div>
          {palace.majorLimit && (
            <div className={`${isSelected ? "text-white/80 dark:text-white/80" : "text-zinc-500 dark:text-zinc-400"}`}>
        ...
        <div className={`flex flex-col items-center justify-center py-0.5 xs:py-1 sm:py-1.5 ${isSelected ? "text-white/80 dark:text-white/80" : "text-zinc-500 dark:text-zinc-400"}`}>
          <div className={`${
            showAnnualFlow ? 
          ...
          <div className="text-2xs xs:text-2xs">
