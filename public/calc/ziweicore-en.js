...
var ziwei = {
  ...
  getLunarDay: function () {
    ...
    return (
      ...
      " " +
      (lunar.l ? this.getTranslation("leap") + " " : "") +
      lunar.m +
      " " +
      this.getTranslation("month") +
      " " +
      lunar.d +
      " " +
      ...
      h +
      " " +
      this.getTranslation("hour")
    );
  },
  getSolarDay: function () {
    // return yS + " 年 " + mS + " 月 " + dS + " 日" + h + " 時";
    return (
      yS +
      " " +
      this.getTranslation("year") +
  ...
  getYinYangGender: function () {
    return (
      YinYang[y1Pos % 2] +
      (g == "M" ? this.getTranslation("male") : this.getTranslation("female"))
    );
  },
  //CenterPart↑
  ...
  putS04Str: function (starName, STAR) {
    return STAR.indexOf(starName) >= 0
      ? "<b>" + StarM_S04[STAR.indexOf(starName)].substring(1, 2) + "</b>"
      : "　";
  },
  ...
  stepSetStar: function (y, m, d, h) {
    ...
    var sG07 = this.getStarArrByPosArr(Star_G07, 7, [
      hPos,
      hPos,
      m - 1,
      m - 1,
    ...
    for (i = 0; i < 12; i++) {
      ...
      for (k = 0; k < 8; k++) {
        if (sT08[k] == i) {
          StarA[lenStar[0]] =
            StarM_A14[k + 6] + this.getS04Str(StarM_A14[k + 6], sS04);
          lenStar[0] += 1;
        }
      }
      //六吉星
      ...
      Place12[i] = {
        MangA:
          HeavenlyStems[((y1Pos % 5) * 2 + ((i < 2 ? i + 2 : i) % 10)) % 10] +
          "<br/>" +
          EarthlyBranches[i],
  ...
  getDaShian: function () {
    ...
    for (i = 0; i < 12; i++) {
      ...
      SiaoPlace =
        g == "M"
          ? (((SiaoShianYear + i) % 12) + 1).toString()
          : (((SiaoShianYear - i + 12) % 12) + 1).toString();
      DShian[((i + lPos) % 12) + 1] =
        DSY.toString() + " - " + (DSY + 9).toString();
